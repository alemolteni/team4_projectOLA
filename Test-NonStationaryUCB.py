...
def choose_learner(step_num, margins, alpha, click_prob, secondary, Lambda, debug, actual_units_mean,
    ...
    if step_num == 3:
        ...
        return UCB_ChangeDetection(margins=margins, secondary=secondary, units_mean=actual_units_mean,
                            clickProbability=click_prob, Lambda=Lambda, alphas=alpha, alpha=0.012)
    else:
...
if MODE == "plots":
    ...
    for i in range(0, len(env)):
        ...
        learner = choose_learner(ucb_type, margins=config_margins[i], alpha=alphas[i], click_prob=click_probs[i],
                                    ...
                                    actual_units_mean=actual_unit_mean[i], sliding_window_size=swin_size)
        ...
        for j in tqdm(range(0, n_experiments)):
            ...
            if ge_margin < 20:
                #if prev_outlier == False: 
                    #print("\nArm with value less than 20 at time {}: {}".format(j, pulledArm))
                    #print("E[REW mat] = {}".format(learner.expected_reward))
                prev_outlier = True
            else:
                prev_outlier = False
        ...
        if ucb_type == 7:
            random_takes = np.full(len(optimal), 0)
            random_takes[learner.random_arm_times] = 7
            axes[i, 0].plot(x, random_takes)

            #resets = np.full(len(optimal), np.nan)
            #print(learner.reset_times)
            #resets[learner.reset_times] = 0
            #axes[i, 1].plot(x, resets)
    ...
    for i in range(0,len(files)):
        ...
        file_name = "./Results/NS-UCB_{}_{}_{}".format(type_name, dtime, config_name)
