...
class UCB_CR():
    ...
    def forwardExpectation (self, prod, test_config, total=0, trace=[], click_prob=1):
        ...
        p = self.clickProbability[prod][next1]
        b = self.clickProbability[prod][next2]
        if self.secondary[prod][0] not in trace and self.secondary[prod][1] not in trace:
            # Case visiting both
            trace.append(next1)
            trace.append(next2)
            self.forwardExpectation(next1, test_config, total=total, trace=trace, click_prob=p * b)
            self.forwardExpectation(next2, test_config, total=total, trace=trace, click_prob=p * b)
            trace.pop()
            trace.pop()
            ...
            # Case only first
            trace.append(next1)
            self.forwardExpectation(next1, test_config, total=total, trace=trace, click_prob=p)
            trace.pop()
        elif self.secondary[prod][0] in trace and self.secondary[prod][1] not in trace:
            # Case only second
            trace.append(next2)
            self.forwardExpectation(next2, test_config, total=total, trace=trace, click_prob=b)
            trace.pop()
